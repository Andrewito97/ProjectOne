...
const userController = {
  ...
  checkIfMediaAccExists(request, response) {
    User.findOne({ email: request.body.email }).exec((error, user) => {
      ...
      if (user.createdWithMedia === 'google') {
        ...
        return response.json({
          isNotMediaAccount:
            'User is created without google or facebook auth !',
        });
      }
    });
  },
  ...
  recoverPassword(request, response) {
    User.findOne({ email: request.body.email }).exec((error, user) => {
      ...
      if (user.createdWithMedia === 'google') {
        return response.status(401).json({
          emailError:
            'This email is registered with google so it does not require password to login !',
        });
      }
      ...
      if (user.createdWithMedia === 'facebook') {
        return response.status(401).json({
          emailError:
            'This email is registered with facebook so it does not require password to login !',
        });
      }
  ...
  updateUser(request, response) {
    User.findByIdAndUpdate(
      ...
      (error, oldUser) => {
        let noChanges =
          request.body.name === oldUser.name &&
  ...
  deleteUser(request, response) {
    User.findByIdAndDelete(request.profile._id, (error) => {
      if (error) {
